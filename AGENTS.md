...
| 公開の流れ | `main` に入ると GitHub Actions が自動でビルドしてさくらへ転送する（実行するのは日本信達の Mac mini。さくらが海外からの転送を受け付けないため） |
...
0. **作業を始める前に** `git switch main` → `git pull` で最新にしておく（日本信達側の修正とぶつからないように）
...
4. マージすると数分で本番サイトに出る。反映されない場合は GitHub の Actions タブを見る。
   「待機中（Queued）」のまま進まないときは Mac mini が止まっているので、日本信達に連絡する
